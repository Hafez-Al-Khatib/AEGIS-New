...
def read_vitals(range: str = "-1h", current_user: models.User = Depends(get_current_user)):
    ...
    """
    try:
        results = influx_manager.read_vitals(
            user_id=current_user.id,
            time_range=range
        )
        return results
...
async def analyze_document(
    ...
    1. Ingestor (Vision) -> JSON
    2. Sentinel (Reasoning) -> Advice
    """
    ...
    try:
        # Agent 1: Vision
        extracted_data = ingestor.extract_medical_data(temp_path)
        
        # Agent 2: Reasoning
        user_ctx = f"User: {current_user.email}"
        analysis = sentinel.analyze_health_record(extracted_data, user_ctx)
